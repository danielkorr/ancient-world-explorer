...
function showPanel(site) {
  ...
  // vici.org — René Voorburg's crowdsourced atlas. Only ~54 sites cross-link to
  // it (from VICI_LINKS, generated by scripts/build-vici-links.mjs off the same
  // Pleiades references), so the button is conditional. has_photo, when present,
  // lets us hint that vici already has imagery for the site.
  const vici = (window.VICI_LINKS && site.pleiades) ? window.VICI_LINKS[site.pleiades] : null;
  const viciBtn = vici ? `
    <a href="${vici.url}" onclick="saveReturnState()" class="p-btn p-btn-vici">
      <span class="p-btn-icon">🏛️</span>
      <div><div class="p-btn-main">Vici.org Atlas</div><div class="p-btn-sub">Community archaeology · plans &amp; photos${vici.has_photo ? ' (has images)' : ''}</div></div>
