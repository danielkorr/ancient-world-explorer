...
async function loadStops() {
  ...
  for (const chunk of block.split(/\n  \{/).slice(1)) {
    ...
    if (id && pl) stops.push({ id, pleiades: pl, name: nm || id });
  }
  return stops;
}
...
async function main() {
  ...
  console.log(`✓ ${stops.length} Alexander stops carry a Pleiades id`);
  ...
  for (const s of stops) {
    ...
    if (rec && rec.image) fileByPid[s.pleiades] = rec.image;
    else needResolve.push(s);
  }
  console.log(`  ${Object.keys(fileByPid).length} already have a cached P18 filename; resolving ${needResolve.length} more`);
  ...
  for (const [pid, qid] of Object.entries(qidByPid)) {
    if (qid && p18[qid]) fileByPid[pid] = p18[qid];
  }
  ...
  for (const s of stops) {
    ...
    if (ci && ci.thumb) {
      ...
      noImg++;
      console.log(`  ○ no image: ${s.name} (${s.pleiades})`);
    }
  }
