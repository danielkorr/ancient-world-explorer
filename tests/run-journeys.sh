...
nav_ready() {
  ...
  for attempt in 1 2 3 4; do
    ...
    [ "$ready" = "1" ] && return 0
    echo "  (${1}: attempt $attempt not ready, re-navigating)"
  done
  return 1
}

# Run the journey at the current viewport and assert the JSON verdict.
run_pass() {
  local label="$1"
  nav_ready "$label" || { echo "FAIL: app never reported ready ($label)"; exit 1; }
  local verdict; verdict="$("$B" eval tests/journey.eval.js 2>&1 | tail -1)"
  ...
  node -e '
    ...
    if (v.failed === 0) { console.log(process.argv[2] + ": ALL JOURNEYS PASSED"); process.exit(0); }
